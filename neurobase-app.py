...
if __name__ == "__main__":
        ...
        if st.button("Gerar Relatório de Evolução"):
            if paciente_id and len(opcoes_relatorio) > 0:
                ...
                if "Dados pessoais" in opcoes_relatorio:
                    ...
                    if paciente['nivel_suporte'] and str(paciente['nivel_suporte']) != 'nan':
                ...
                if "Habilidades trabalhadas" in opcoes_relatorio and len(atendimentos_periodo) > 0:
                    ...
                    for i, atendimento in atendimentos_periodo.iterrows():
                        if isinstance(atendimento['habilidades_trabalhadas'], str):
        ...
        if st.button("Analisar Habilidades"):
            ...
            if len(atendimentos_periodo) > 0:
                ...
                if habilidades_contagem:
                    ...
                    habs = [item['habilidade'] for item in dados_comparacao]
                    prioridades = [item['prioridade'] for item in dados_comparacao]
                    frequencias = [item['frequencia'] for item in dados_comparacao    with tabs[3]:
        st.markdown("<h2 class='sub-header'>Compartilhar Relatórios via WhatsApp</h2>", unsafe_allow_html=True)
    ...
    with tabs[3]:
        ...
        with col2:
            ...
            incluir_anexos = st.checkbox("Incluir anexos (gráficos e tab    with tabs[1]:
        st.markdown("<h2 class='sub-header'>Relatório de Frequência</h2>", unsafe_allow_html=True)
...
def mostrar_configuracoes():
    ...
    tabs = st.tabs(["Usuários", "Terapeutas", "Testes e Avaliações", "Integrações", "Backup e Restauração"])
    
    with tabs[0]:
        st.markdown("<h2 class='sub-header'>Gestão de Usuários</h2>", unsafe_allow_html=True)
        
        # Listagem de usuários
        st.markdown("### Usuários Cadastrados")
        
        # Exibir usuários do sistema de autenticação
        usuarios = []
        for username, data in config['usernames'].items():
            usuarios.append({
                'username': username,
                'nome': data['name'],
                'email': data['email']
        ...
        with st.form("form_novo_usuario"):
            ...
            nivel_acesso = st.selectbox(
                "Nível de Acesso",
                options=["Administrador", "Terapeuta", "Recepção", "Visualização"]
            )
            
            submitted = st.form_submit_button("Adicionar Usuário")
            
            if submitted:
                if novo_username and novo_nome and novo_email and nova_senha:
                    st.success(f"Usuário {novo_username} adicionado com sucesso! (Simulação)")
                else:
                    st.error("Por favor, preencha todos os campos.")
    
    with tabs[1]:
        st.markdown("<h2 class='sub-header'>Gestão de Terapeutas</h2>", unsafe_allow_html=True)
        
        # Simulação de lista de terapeutas
        terapeutas = [
            {"nome": "Ana Silva", "especialidade": "Psicomotricista", "status": "Ativo"},
            {"nome": "Carlos Oliveira", "especialidade": "Terapeuta ABA", "status": "Ativo"},
            {"nome": "Mariana Santos", "especialidade": "Psicomotricista", "status": "Ativo"},
            {"nome": "Ricardo Ferreira", "especialidade": "Psicomotricista", "status": "Inativo"}
        ]
        
        terapeutas_df = pd.DataFrame(terapeutas)
        st.dataframe(terapeutas_df)
        
        # Formulário para adicionar novo terapeuta
        st.markdown("### Adicionar Novo Terapeuta")
        
        with st.form("form_novo_terapeuta"):
            col1, col2 = st.columns(2)
            ...
            with col1:
                ...
                especialidade = st.selectbox(
                    "Especialidade",
                    options=["Psicomotricista", "Terapeuta ABA", "Fonoaudiólogo", "Psicólogo", "Terapeuta Ocupacional", "Outro"]
                )
            
            with col2:
                registro = st.text_input("Registro profissional")
                status = st.selectbox("Status", options=["Ativo", "Inativo"])
            
            horarios_disponiveis = st.text_area("Horários disponíveis")
            
            submitted = st.form_submit_button("Adicionar Terapeuta")
            
            if submitted:
                if nome_terapeuta and especialidade:
                    st.success(f"Terapeuta {nome_terapeuta} adicionado com sucesso! (Simulação)")
                else:
                    st.error("Por favor, preencha os campos obrigatórios.")
    
    with tabs[2]:
        st.markdown("<h2 class='sub-header'>Configuração de Testes e Avaliações</h2>", unsafe_allow_html=True)
        
        # Lista de testes configurados
        st.markdown("### Testes Configurados")
        ...
        for tipo in TIPOS_AVALIACAO:
            ...
            with st.expander("Ver detalhes"):
                st.markdown("**Descrição:** Teste para avaliação psicomotora específica.")
                st.markdown("**Periodicidade recomendada:** Anual")
                st.markdown("**Campos personalizados:** Configurados")
            
            col1, col2 = st.columns([1, 4])
            with col1:
                st.button("Editar", key=f"edit_{tipo}")
            st.markdown("</div>", unsafe_allow_html=True)
        
        # Adicionar novo teste
        st.markdown("### Adicionar Novo Teste")
        
        with st.form("form_novo_teste"):
            nome_teste = st.text_input("Nome do teste")
            ...
            with col1:
                periodicidade = st.selectbox(
                    ...
                    options=["Mensal", "Trimestral", "Semestral", "Anual", "Personalizado"]
                )
            
            with col2:
                if periodicidade == "Personalizado":
                    dias = st.number_input("Dias", min_value=1, value=180)
            ...
            submitted = st.form_submit_button("Adicionar Teste")
            
            if submitted:
                if nome_teste and descricao:
                    st.success(f"Teste {nome_teste} adicionado com sucesso! (Simulação)")
                else:
                    st.error("Por favor, preencha os campos obrigatórios.")
    
    with tabs[3]:
        st.markdown("<h2 class='sub-header'>Integrações</h2>", unsafe_allow_html=True)
        ...
        with col1:
            ...
            st.markdown("Status: Não configurado")
            if st.button("Configurar Google Calendar"):
                st.info("Simulação: Processo de autenticação iniciado.")
            st.markdown("</div>", unsafe_allow_html=True)
            
            st.markdown("<div class='card'>", unsafe_allow_html=True)
            st.markdown("#### Microsoft Excel")
            st.markdown("Status: Ativo")
            if st.button("Testar Conexão Excel"):
                st.success("Conexão com Excel testada com sucesso! (Simulação)")
            st.markdown("</div>", unsafe_allow_html=True)
        
        with col2:
            st.markdown("<div class='card'>", unsafe_allow_html=True)
            st.markdown("#### API WhatsApp Business")
            st.markdown("Status: Não configurado")
            if st.button("Configurar WhatsApp Business"):
                st.info("Simulação: Processo de configuração WhatsApp iniciado.")
            st.markdown("</div>", unsafe_allow_html=True)
            
            st.markdown("<div class='card'>", unsafe_allow_html=True)
            st.markdown("#### API de Email")
            st.markdown("Status: Ativo")
            if st.button("Testar Envio de Email"):
                st.success("Email de teste enviado com sucesso! (Simulação)")
            st.markdown("</div>", unsafe_allow_html=True)
        
        # Configuração de novos conectores
        st.markdown("### Adicionar Nova Integração")
        
        tipo_integracao = st.selectbox(
            "Tipo de Integração",
            options=["API Personalizada", "Banco de Dados Externo", "Outro Sistema", "Serviço na Nuvem"]
        )
        
        with st.form("form_nova_integracao"):
            nome_integracao = st.text_input("Nome da Integração")
            url_endpoint = st.text_input("URL/Endpoint")
            
            col1, col2 = st.columns(2)
            
            with col1:
                chave_api = st.text_input("Chave API/Token", type="password")
            
            with col2:
                metodo = st.selectbox("Método", options=["GET", "POST", "PUT", "DELETE"])
            
            parametros = st.text_area("Parâmetros (JSON)")
            
            submitted = st.form_submit_button("Adicionar Integração")
            
            if submitted:
                if nome_integracao and url_endpoint:
                    st.success(f"Integração {nome_integracao} adicionada com sucesso! (Simulação)")
                else:
                    st.error("Por favor, preencha os campos obrigatórios.")
    ...
    with tabs[4]:
        ...
        with col1:
            ...
            opcoes_backup = st.multiselect(
                "Selecione os dados para backup",
                options=["Pacientes", "Atendimentos", "Avaliações", "Agendamentos", "Configurações do Sistema"],
                default=["Pacientes", "Atendimentos", "Avaliações", "Agendamentos"]
            )
            
            formato = st.radio("Formato do Backup", options=["Completo (.zip)", "Apenas Dados (.csv)", "Apenas Configurações"])
            ...
            if st.button("Gerar Backup"):
                if opcoes_backup:
                    ...
                    st.download_button(
                        ...
                        data="Simulação de arquivo de backup",
                        file_name=f"backup_neurobase_{datetime.date.today().strftime('%Y%m%d')}.zip",
                        mime="application/zip"
                    )
                else:
                    st.error("Selecione pelo menos uma opção para o backup.")
        
        with col2:
            st.markdown("### Restaurar Backup")
            
            backup_file = st.file_uploader("Selecione o arquivo de backup", type=["zip", "csv", "json"])
            
            opcoes_restauracao = st.multiselect(
                "Dados a restaurar",
                options=["Pacientes", "Atendimentos", "Avaliações", "Agendamentos", "Configurações do Sistema"],
                default=["Pacientes", "Atendimentos", "Avaliações", "Agendamentos"]
            )
            ...
            if st.button("Restaurar Backup"):
                if backup_file and opcoes_restauracao:
                    ...
                    st.success("Backup restaurado com sucesso! (Simulação)")
                else:
                    st.error("Selecione um arquivo de backup e pelo menos uma opção para restaurar.")
            
            st.warning("Atenção: A restauração de backup irá substituir os dados atuais pelos dados do backup. Esta operação não pode ser desfeita.")
    ...
    with tabs[0]:
        ...
        if st.button("Gerar Relatório de Evolução"):
            if paciente_id and len(opcoes_relatorio) > 0:
                ...
                if "Recomendações            # Entre 30 e 60 dias
            st.markdown("### Reavaliações entre 30 e 60 dias")
            entre_30_60 = avaliacoes_pendentes[
                (avaliacoes_pendentes['proxima_avaliacao'] > proximos_30_dias.strftime('%Y-%m-%d')) &
                (avaliacoes_pendentes['proxima_avaliacao'] <= proximos_60_dias.strftime('%Y-%m-%d'))
            ]
            
            if len(entre_30_60) > 0:
                for i, avaliacao in entre_30_60.iterrows():
                    col1, col2, col3 = st.columns([3, 2, 1])
                    
                    with col1:
                        st.markdown(f"**{avaliacao['nome']}**")
                    
                    with col2:
                        st.markdown(f"{avaliacao['tipo_avaliacao']}")
                    
                    with col3:
                        data_prox = datetime.datetime.strptime(avaliacao['proxima_avaliacao'], '%Y-%m-%d').date()
                        dias_restantes = (data_prox - hoje).days
                        st.markdown(f"**{avaliacao['proxima_avaliacao']}** ({dias_restantes} dias)")
            else:
                st.info("Não há reavaliações entre 30 e 60 dias.")
        else:
            st.info("Não há reavaliações programadas para os próximos 60 dias.")
...
def mostrar_agendamentos():
    ...
    with tabs[1]:
        ...
        # Em uma implementação real, usaríamos uma biblioteca JS para isso
        ...
        if len(agendamentos_filtrados) > 0:
            ...
            for i, agendamento in agendamentos_completos.iterrows():
                ...
                with col3:
                    ...
                    if agendamento['observacao'] and str(agendamento['observacao']) != 'nan':
            ...
            if 'editar_agendamento' in st.session_state and st.session_state.editar_agendamento:
                ...
                with st.form("form_editar_agendamento"):
                    # Campos de edição
                    novo_status = st.selectbox("Status", options=["Agendado", "Confirmado", "Em andamento", "Concluído", "Cancelado", "Faltou"], index=["Agendado", "Confirmado", "Em andamento", "Concluído", "Cancelado", "Faltou"].index(agendamento['status']))
...
def mostrar_avaliacoes():
    ...
    tabs = st.tabs(["Nova Avaliação", "Histórico de Avaliações", "Alertas de Reavaliação"])
    
    with tabs[0]:
        st.markdown("<h2 class='sub-header'>Registrar Nova Avaliação</h2>", unsafe_allow_html=True)
        
        # Formulário de registro de avaliação
        with st.form("form_avaliacao"):
            ...
            with col1:
                ...
                terapeuta = st.text_input("Terapeuta Responsável", key="terapeuta_avaliacao")
            
            with col2:
                tipo_avaliacao = st.selectbox("Tipo de Avaliação", TIPOS_AVALIACAO)
                proxima_avaliacao = st.date_input(
            ...
            resultados = st.text_area("Descreva os resultados da avaliação", height=200)
            
            st.markdown("### Recomendações")
            recomendacoes = st.text_area("Recomendações terapêuticas", height=150)
            
            submitted = st.form_submit_button("Registrar Avaliação")
            
            if submitted:
                # Verificar dados obrigatórios
                if not paciente_id or not data_avaliacao or not tipo_avaliacao:
                    st.error("Por favor, preencha os campos obrigatórios.")
                else:
                    # Criar novo ID
                    novo_id = 1
                    if len(avaliacoes_df) > 0:
                        novo_id = int(avaliacoes_df['id'].max()) + 1
                    ...
                    avaliacoes_df.loc[len(avaliacoes_df)] = nova_avaliacao
                    avaliacoes_df.to_csv("dados/avaliacoes.csv", index=False)
    ...
    with tabs[1]:
        st.markdown("<h2 class='sub-header'>Histórico de Avaliações</h2>", unsafe_allow_html=True)
        ...
        with col1:
            paciente_filtro = st.selectbox(
                ...
                options=[0] + pacientes_df['id'].tolist(),
                format_func=lambda x: "Todos os pacientes" if x == 0 else pacientes_df[pacientes_df['id'] == x]['nome'].iloc[0] if x in pacientes_df['id'].values else "",
                key="filtro_aval_paciente"
            )
        
        with col2:
            tipo_filtro = st.multiselect("Filtrar por Tipo de Avaliação", TIPOS_AVALIACAO)
        
        # Aplicar filtros
        if paciente_filtro != 0:
            avaliacoes_filtradas = avaliacoes_df[avaliacoes_df['paciente_id'] == paciente_filtro]
        else:
            avaliacoes_filtradas = avaliacoes_df.copy()
        
        if tipo_filtro:
            avaliacoes_filtradas = avaliacoes_filtradas[avaliacoes_filtradas['tipo_avaliacao'].isin(tipo_filtro)]
        ...
        if len(avaliacoes_filtradas) > 0:
            ...
            # Juntar com nomes dos pacientes
            avaliacoes_completas = avaliacoes_filtradas.merge(
                pacientes_df[['id', 'nome']], 
                left_on='paciente_id', 
                right_on='id', 
                suffixes=('', '_paciente')
            ...
            for i, avaliacao in avaliacoes_completas.iterrows():
                st.markdown(f"<div class='card'>", unsafe_allow_html=True)
                st.markdown(f"#### {avaliacao['data']} - {avaliacao['nome']}")
                st.markdown(f"**Tipo de Avaliação**: {avaliacao['tipo_avaliacao']}")
                st.markdown(f"**Terapeuta**: {avaliacao['terapeuta']}")
                
                with st.expander("Ver resultados da avaliação"):
                    st.markdown("**Resultados:**")
    ...
    with tabs[2]:
        st.markdown("<h2 class='sub-header'>Alertas de Reavaliação</h2>", unsafe_allow_html=True)
        
        hoje = datetime.date.today()
        proximos_30_dias = hoje + datetime.timedelta(days=30)
        proximos_60_dias = hoje + datetime.timedelta(days=60)
        
        # Buscar avaliações que precisam ser feitas nos próximos 60 dias
        avaliacoes_pendentes = avaliacoes_df[
            (avaliacoes_df['proxima_avaliacao'] >= hoje.strftime('%Y-%m-%d')) & 
            (avaliacoes_df['proxima_avaliacao'] <= proximos_60_dias.strftime('%Y-%m-%d'))
        ]
        
        if len(avaliacoes_pendentes) > 0:
            # Juntar com dados dos pacientes
            avaliacoes_pendentes = avaliacoes_pendentes.merge(
                pacientes_df[['id', 'nome']], 
                left_on='paciente_id', 
                right_on='id', 
                suffixes=('', '_paciente')
            ...
            # Próximos 30 dias
            st.markdown("### Reavaliações nos próximos 30 dias")
            proximos_30 = avaliacoes_pendentes[
                avaliacoes_pendentes['proxima_avaliacao'] <= proximos_30_dias.strftime('%Y-%m-%d')
            ]
            
            if len(proximos_30) > 0:
                for i, avaliacao in proximos_30.iterrows():
                    col1, col2, col3 = st.columns([3, 2, 1])
                ...
                st.info("Não há reavaliações nos próximos 30 dias.")
    ...
    with tabs[2]:
        if 'paciente_selecionado' in st.session_state:
            ...
            with col1:
                if paciente['foto'] and str(paciente['foto']) != 'nan':
                    try:
            ...
            with paciente_tabs[0]:
                ...
                if len(atendimentos_paciente) > 0:
                    ...
                    for i, atendimento in atendimentos_paciente.iterrows():
                        ...
                        st.markdown("**Habilidades trabalhadas:**")
                        habilidades_list = atendimento['habilidades_trabalhadas'].split(',') if isinstance(atendimento['habilidades_trabalhadas'], str) else []
                        ...
                        if isinstance(atendimento['comportamentos'], str) and atendimento['comportamentos']:
...
# Interface principal
def main():
    autenticado, username = autenticacao()
    
    if not autenticado:
        st.markdown("<h1 class='main-header'>NeuroBase</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center'>Sistema de Gestão em Psicomotricidade para Clínicas ABA</p>", 
                  unsafe_allow_html=True)
        return
    
    # Sidebar para navegação
    with st.sidebar:
        st.image("https://via.placeholder.com/150x150.png?text=NeuroBase", width=150)
        st.markdown(f"**Olá, {config['usernames'][username]['name']}!**")
                ...
                for i, paciente in pacientes_filtrados.iterrows():
                    ...
                    with col1:
                        if paciente['foto'] and str(paciente['foto']) != 'nan':
                            ...
                            st.image("https://via.placeholder.com/100x100.png            # Determinar período de análise
            hoje = datetime.date.today()
            if periodo_hab == "Último mês":
                data_inicio = hoje - datetime.timedelta(days=30)
            elif periodo_hab == "Últimos 3 meses":
...
def mostrar_agendamentos():
    ...
    with tabs[1]:
        ...
        cols = st.columns(7)
        for i, dia in enumerate(dias_semana):
            cols[i].markdown(f"<div style# Função para carregar ou criar dados
def load_data():
...
def mostrar_dashboard():
    ...
    # Tabela de agendamentos do dia
    agendamentos_hoje = agendamentos_df[agendamentos_df['data'] == datetime.date.today().strftime('%Y-%m-%d')]
    if len(agendamentos_hoje) > 0:
        agendamentos_hoje = agendamentos_hoje.merge(
            pacientes_df[['id', 'nome']], 
            left_on='paciente_id', 
...
def mostrar_agendamentos():
    ...
    with tabs[2]:
        ...
        with col3:
            filtro_status = st.multiselect(
                "Status",
                options=["Agendado", "Confirmado", "Em andamento", "Concluído", "Cancelado", "F